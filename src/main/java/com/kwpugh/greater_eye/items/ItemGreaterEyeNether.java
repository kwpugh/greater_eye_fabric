...
public class ItemGreaterEyeNether extends Item
{
	...
	public TypedActionResult<ItemStack> use(World worldIn, PlayerEntity playerIn, Hand handIn)
	{
		...
		if(!worldIn.isClient)
		{
			...
			if((worldIn instanceof ServerWorld) && (playerIn.isSneaking() && (serverWorld.getRegistryKey().equals(World.NETHER))))    //shift right-click changes structure type to locate
			{
		...
		if(!playerIn.isSneaking())   //simple right-click executes
		{
			if(!worldIn.isClient)
			{
				...
				if((worldIn instanceof ServerWorld) && (serverWorld.getRegistryKey().equals(World.NETHER)));
				{
